...
Public Class CtlDeviceSetting
    ...
    Private Sub SelectPanelTabIndex(ByVal index As Integer)
        ...
        SyncLock selectCtlLock
            If index = 0 Then '设备列表          
                Panel5.Controls.Clear()
                Dim ctl As New AllDeviceList
                Panel5.Controls.Add(ctl)
            End If
            If index = 1 Then '设备网关
                Panel5.Controls.Clear()
                Dim ctl As New TSSGateWayListPanel
                Panel5.Controls.Add(ctl)
            End If
            If index = 2 Then '监测网关
                Panel5.Controls.Clear()
                Dim ctl As New INGDeviceList
                Panel5.Controls.Add(ctl)
            End If
            If index = 3 Then '设备设置
                Panel5.Controls.Clear()
                Dim ctl As New DeviceSetting
                Panel5.Controls.Add(ctl)
            End If
        End SyncLock
    End Sub
