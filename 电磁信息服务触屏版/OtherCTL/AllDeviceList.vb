...
#End Region
    ...
    Private Sub LVDetail_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles LVDetail.SelectedIndexChanged
        ...
        ' frmMain.SelectDevice(selectDeviceID)
        If lng <> "" And lat <> "" Then
            setGisCenter3(lng, lat, 15, WebGis)
            'frmMain.selectDeviceID = selectDeviceID
            'frmMain.Label2.Text = "选中  " & "微型传感器" & "  " & selectDeviceID
            'If selectDeviceKind = "微型传感器" Then
            '    frmMain.p_beClick("微型传感器," & selectDeviceID)
            'End If
            'If selectDeviceKind = "频谱传感器" Then
            '    frmMain.p_beClick("频谱传感器," & selectDeviceID)
            'End If
        End If
    ...
    Private Sub 选择该传感器ToolStripMenuItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles 选择该传感器ToolStripMenuItem.Click
        ...
        Dim isOnline As String = itm.SubItems(4).Text
        If isOnline <> "在线" Then Return
        'frmMain.SelectDevice(DeviceName)
        'frmMain.isSelected = True
        'frmMain.selectDeviceID = DeviceName
        'If Kind = "微型传感器" Then frmMain.selectDeviceKind = "TZBQ"
        'If Kind = "频谱传感器" Then frmMain.selectDeviceKind = "TSS"
        'frmMain.Label2.Text = "选中  " & "微型传感器" & "  " & DeviceName
    End Sub
